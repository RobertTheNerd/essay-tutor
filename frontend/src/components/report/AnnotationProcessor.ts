...
export class AnnotationProcessor {
  ...
  generateMockAnnotations(text: string): ProcessedAnnotation[] {
    ...
    sentences.forEach((sentence, index) => {
      if (index < 8 && sentence.length > 50) { // Focus on substantial sentences
        ...
        if (startIndex !== -1) {
          annotations.push({
            ...
            explanation: this.generateExplanation(category, highlight.text),
            suggestion: this.generateSuggestion(category, highlight.text),
  ...
  private selectCategoryForSentence(sentence: string, index: number): string {
    ...
    if (sentence.includes('because') || sentence.includes('therefore') || sentence.includes('however')) {
      return 'structure';
    }
    if (sentence.length > 100) {
      return 'development';
    }
    if (/[A-Z][a-z]{8,}/.test(sentence)) { // Long words
      return 'vocabulary';
    }
  ...
  private generateExplanation(category: string, _text: string): string {
  ...
  private generateSuggestion(category: string, _text: string): string {
