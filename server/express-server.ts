...
import { handleProcessText, handleUploadMultiple, handleHello } from './lib/handlers.js'
...
const server = app.listen(PORT, () => {
  ...
  const hasAzure = !!(process.env.AZURE_OPENAI_ENDPOINT && process.env.AZURE_OPENAI_DEPLOYMENT_NAME)
  
  if (hasAzure) {
    console.log(`🤖 AI: Azure OpenAI configured (${process.env.AZURE_OPENAI_DEPLOYMENT_NAME})`)
  } else if (hasOpenAI) {
    console.log(`🤖 AI: OpenAI configured`)
  } else {
    console.log(`⚠️  AI: No API key configured - set OPENAI_API_KEY or Azure OpenAI environment variables`)
  }
})
